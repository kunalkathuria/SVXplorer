    ...
    for al1 in list1:
		...
		if MatchF2 > 0:
			MatchR_2 = float(NMatch2)/float(MatchF2)
                ...
                if NMatch1 > MATCH_PCT*RDL and MatchR_1 >= MATCH_THRESH and ASF1 > AS_THRESH* RDL and Al_S_1 >= PCT_THRESH*ASF1:
			al1_match = 1

                else:
                    continue # can use return here if sure BAM file reports alignments in order of decreasing AS
		
                if NMatch2 > MATCH_PCT*RDL and MatchR_2 >= MATCH_THRESH and ASF2 > AS_THRESH* RDL and Al_S_2 >= PCT_THRESH*ASF2:
                    	al2_match = 1
