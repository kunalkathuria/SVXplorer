...
def calcEdgeWeight(f1lpos, f1rpos, f2lpos, f2rpos, totalent, c_type):
	...
	distpen = 1 + (pen_atmargin - 1)*(abs(f2lpos-f1lpos)+2*RDL-DIST_PEN)*1.0/(DISC_D - DIST_PEN)
...
if __name__== "__main__":
	...
	# refresh list	
		...
		elif temp.ltid == FList[newBlockS].ltid and (temp.l_bound - FList[newBlockS].l_bound) > CLUSTER_D:
    ...
    max_clique_list = []
    for connected_comp in list(nx.connected_components(FG)):
