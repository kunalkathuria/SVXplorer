...
def writeClusters(G, fragHash, fc, ft, f3, f4, preserveList):
	...
	for node in elem:
		...
		ncnnxns = 0
		avgWeightL = computeAW(G, node, elem)
		maxWeight = avgWeightL
		for h,elem2 in enumerate(max_clique_list2):
			#print nx.cliques_containing_node(G,node,[elem2]), len(nx.cliques_containing_node(G,node,[elem2]))
			if len(nx.cliques_containing_node(G,node,[elem2])) > 0 and elem2 >= MIN_CLUSTER_SIZE:
				avgWeight,ncnnxnsL = computeAW(G, node, elem2)
				ncnnxns+= ncnnxnsL
				if avgWeight > maxWeight:
					maxWeight = avgWeight
					maxIndex = h
				if ncnnxns >= G.degree(node):
					break
		if maxWeight > avgWeightL:
			fragHash[node].reserved = maxIndex
	...
	rmax = fragHash[elem0].r_bound + 1
        ...
        if len(elem) >= MIN_CLUSTER_SIZE:
		   ...
		   if item_s not in pickedFrags and not fragHash[item].used and (fragHash[item].reserved ==-1 or fragHash[item].reserved == k):
