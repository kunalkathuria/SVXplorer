...
def findTotalNMatches(al):
	...
	#if not found, then perfect match it seems 
	if MDtag_pos == 5:
    ...
    for al1 in list1:
		...
		if MatchF2 > 0:
			MatchR_2 = float(NMatch2)/float(MatchF2)
                ...
                if NMatch1 > MATCH_PCT*al1.infer_query_length() and MatchR_1 >= MATCH_THRESH and ASF1 > AS_THRESH*al1.infer_query_length() and Al_S_1 >= PCT_THRESH*ASF1:
			al1_match = 1

                else:
                    continue # can use return here if sure BAM file reports alignments in order of decreasing AS
		
                if NMatch2 > MATCH_PCT*al2.infer_query_length() and MatchR_2 >= MATCH_THRESH and ASF2 > AS_THRESH*al2.infer_query_length() and Al_S_2 >= PCT_THRESH*ASF2:
                    	al2_match = 1
		...
		CLType = str(int(l_orient)) + str(int(r_orient))
			...
			if left_tid == right_tid and OUTER_D - MEAN_D < -3*SIG_D:
				Cl_small = 1
