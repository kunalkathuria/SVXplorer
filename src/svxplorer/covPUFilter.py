...
from utils import readChromosomeLengths
...
def calculateLocCovg(NH_REGIONS_FILE, chr_n, bpFirst, bpSecond, PILEUP_THRESH, fBAM, chrHash,
    ...
    if stop > start and ((not isTD) or (stop - start < MAX_TD_SIZE)):
        ...
        if bpSecond - bpFirst >= MIN_BP_SPAN:
            ...
            for pileupcolumn in fBAM.pileup(chr_n, start, stop, stepper="all", truncate=True):
                ...
                if NH_REGIONS_FILE is not None and \
                        (chr_n in chrHash and pileupcolumn.pos < len(chrHash[chr_n]) and chrHash[chr_n][
                            pileupcolumn.pos]):
                    covLoc += puVal
                    counter += 1
                    covLoc -= badReadCount
    ...
    logging.debug("Good bases found, confirmation, covGood final, chr cov: %s, %s, %s, %s", gbCount, confRegion,
                  covLocG / (.01 + gbCount), covHash[chr_n])
...
def covPUFilter(workDir, avFile, vmFile, ufFile, statFile, bamFile,
    ...
    fAVN.write(
        "VariantNum\tType\tchr1\tstart1\tstop1\tchr2\tstart2\tstop2\tchr3\tstart3\tstop3\t SupportBy\tNPEClusterSupp\tNFragPESupp\tNFragSRSupp\tSwapBP\tBNDFlag\tSupport\tGT\n")
    ...
    logging.info(
        "DEL_THRESH, DEL_THRESH_S, DUP_THRESH, DUP_THRESH_S, MIN_PILEUP_THRESH, MIN_PILEUP_THRESH_NH: %s, %s, %s, %s, %s, %s",
        DEL_THRESH, DEL_THRESH_S, DUP_THRESH, DUP_THRESH_S, MIN_PILEUP_THRESH, MIN_PILEUP_THRESH_NH)
            ...
            if lineAV_split[11].find("RD") == -1:
                ...
                if (svtype == "DEL_INS" or svtype == "DEL" or svtype[0:2] == "TD") and \
                    ...
                    covLocM, confMiddle, largeDupRet = calculateLocCovg(NH_REGIONS_FILE, lineAV_split[2],
                                                                        ...
                                                                        [int(lineAV_split[3]), int(lineAV_split[7])],
                    ...
                    if svtype.startswith("TD") and lineAV_split[11].find("PE") == -1 and \
                        ...
                        logging.debug(
                            "Rejected %s due to low cvg or insufficient evidence of coverage due to small variant region",
                            lineAV)
                        ...
                        if covLocM < DEL_THRESH_GT:
                            ...
                            GT = "GT:0/1"
                                                       ...
                                                       (lineAV_split[11].find("PE") == -1 and (
                                                               confMiddle == 0 or covLocM > DEL_THRESH))):
                        logging.debug(
                            "Rejected %s due to high cvg or insufficient evidence of coverage due to small variant region",
                            lineAV)
                        # since bp3 = -1, this will be written as a BND event
                        svtype = "INS_halfFR"
                    ...
                    covLoc_23, conf_23, _ = calculateLocCovg(NH_REGIONS_FILE, lineAV_split[5],
                                                             ...
                                                             [int(lineAV_split[6]), int(lineAV_split[10])],
                                                             MIN_PILEUP_THRESH, MIN_PILEUP_THRESH_NH, isTD)
                    covInfo = covLoc_23
...
def main():
    PARSER = argparse.ArgumentParser(
        description='Filter variants based on local coverage in variant regions and write final SVs in BED format')
    ...
    PARSER.add_argument('bamFile', help='Position-sorted args.BAM file for sample')  # TODO args.BAM ?
