...
class newSRVar(object):
	...
	def __str__(self):
 	       ...
 	       return "%s %s %s" %(self.bp2, self.bp3, self.count)
...
if __name__ == "__main__":
	...
	if match:
		...
		if SRCheck:
			...
			#if SRVarHash[temp].typeO == "TD_I":
				#print temp, SRVarHash[temp]
			...
			#Also, INS_I would be off by ~.5 RDL or less due to ambiguity of swapped alignment: whether ref start or end alignment point is breakpoint
				...
				# cannot update type based on this info	
					...
					#following gave many FPs with lumpy-extracted SRs at least due to funny alignments
	...
	if SRVarHash[item].count > 0:
		...
		#if SRVarHash[item].typeO == "TD_I":
			#print item, SRVarHash[item]
		...
		if SRVarHash[item].write == 1 and SRVarHash[item].count >= MIN_CS_SR:
			...
			if (SRVarHash[item].typeO == "DEL" or SRVarHash[item].typeO == "TD_I" or SRVarHash[item].typeO == "INV" or SRVarHash[item].typeO == "INV_POSS") and item.bp > SRVarHash[item].bp2:
				fp3.write("%s %s %s %s %s %s %s %s %s %s %s %s %s\n" %(k+varNum+1,SRVarHash[item].typeO, item.tid_2, SRVarHash[item].bp2-SR_SLOP/2, SRVarHash[item].bp2+SR_SLOP/2, item.tid, item.bp-SR_SLOP/2, item.bp + SR_SLOP/2, SRVarHash[item].bp3tid, SRVarHash[item].bp3, SRVarHash[item].bp3, "SR", SRVarHash[item].count))
