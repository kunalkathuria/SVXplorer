...
if __name__== "__main__":
	...
	for k,line2 in enumerate(f):
		...
		if lmax - lmin + RDL >= RLENGTH:
			start = stop - (lmax - lmin + RDL) # RLENGTH
		else:
			start = -1
		...
		if start > 0 and line2_split[2] == "01":
			for read in samfile.fetch(chr_n, start, stop):
				...
				if read.is_proper_pair and (start <= read.reference_start <= stop - SPLITM) and read.is_read1 > 0 and not read.is_secondary and not read.is_supplementary:
					n_frags_c+=1
