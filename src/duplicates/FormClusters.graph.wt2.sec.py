...
def writeClusters(G, fragHash, fc, ft, f3, f4, preserveList):
	...
	#connected_comp = sorted(connected_comp_US, key = G.degree, reverse= True)
	preserveComp = 0
	if len(connected_comp) >= MIN_CLUSTER_SIZE:
	   ...
	   currentCliquesC = []
		...
		for k,cl in enumerate(currentCliquesC):
			...
			for item in cl:
				if G.has_edge(node,item):
					...
					edgeC = 0
					#print item, currentCliquesC
					#print "Has no cnnxn", node, fragHash[node].ltid,fragHash[node].l_bound, fragHash[node].rtid, fragHash[node].r_bound , G.degree(node), edgeC, item, fragHash[item].ltid, fragHash[item].l_bound, fragHash[item].rtid, fragHash[item].r_bound
					break
		...
		if G.degree(node) > 0:
			fragHash[node].avgW = fragHash[node].totalW/G.degree(node)			

		#print node, fragHash[node].avgW, maxavgW
		#if no cnnxn with existing cliques or if avg node weight larger than avg weight vis-a-vis current clique set
		if edgeC_T == 0 or (fragHash[node].avgW > WCOMP_FACTOR*maxavgW and G.degree(node) - edgeC_T >= MIN_CLUSTER_SIZE - 1):
			currentCliquesC.append([node])
			#print "Starting new clique", node,fragHash[node].ltid, fragHash[node].l_bound, fragHash[node].rtid, fragHash[node].r_bound, fragHash[node].avgW, maxavgW
		else:
			currentCliquesC[maxindex].append(node)
	   		#print node, fragHash[node].ltid, fragHash[node].l_bound, fragHash[node].rtid, fragHash[node].r_bound, "Added to", currentCliquesC[maxindex], fragHash[node].avgW, maxavgW

		#fragHash[node].compIndex = len(currentCliques)
		#compList[fragHash[node].compIndex] = len(currentCliquesC)
...
#	for node in elem:
#		edgeC_T = 0
#		start = fragHash[node].compIndex
#		end = fragHash[node].compIndex + compList[fragHash[node].compIndex]
#		for m,clique in enumerate(currentCliques[start:end]):
#		   if len(clique) >= MIN_CLUSTER_SIZE -1:
#			sumW = 0
#			edgeC = 0
#			for node2 in clique:
#				if G.has_edge(node,node2):
#					sumW = sumW + G[node][node2]['weight']
#					edgeC+=1
#					edgeC_T+=1
#				#else:
#					#edgeC = 0
...
#                                break
#
#		if maxavgW > WFACTOR*fragHash[node].avgW:
#                	currentCliques[maxindex].append(node)
	...
	rmax = fragHash[elem0].r_bound + 1
        ...
        if len(elem) >= MIN_CLUSTER_SIZE:
        	for item in elem:
		   #secondary almts are noted with underscore
		   uspos = item.find("_")
                   if uspos != -1:
		   ...
		   #do not support cluster twice with 2 almts of same fragment
	    ...
	    #print "Member:", FList[x]
 	    if FList[x].ltid == temp.ltid and FList[x].rtid == temp.rtid and FList[x].C_type == temp.C_type and FList[x].clsmall == temp.clsmall:
			...
			if FList[x].fragNum != temp.fragNum:
				...
				fragHash[temp.fragNum].totalW+= EW
