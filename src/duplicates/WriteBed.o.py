		...
		if x[num] > 0:
			...
			if line2_split[1] == "DEL":
				...
				bp3_s = temp 
                            ...
                            f16.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\n" %(line2_split[5], bp2_s, bp3_e, line2_split[2], line2_split[3], line2_split[4], line2_split[1]))

                        # this is read from cluster file, so has 2 bps only; INS_C w/ only 2 clusters supporting; INV w/ only 1
			elif line2_split[1] == "Unknown" or (line2_split[1] == "INS_C" and line2_split[12] == 2) or line2_split[1] == "INV_POSS":
