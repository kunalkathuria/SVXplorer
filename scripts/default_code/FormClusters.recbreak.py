...
def CompareCluster(map_type, DiscordCluster, Cl2):
   ...
   if Cl2.C_type == DiscordCluster.C_type and Cl2.ltid == DiscordCluster.ltid and Cl2.rtid == DiscordCluster.rtid:
 
     if map_type == 0 and ((lPos - DiscordCluster.l_bound_O) < CLUSTER_D + .5*SIG_MULT*SIG_D or abs(lPos - DiscordCluster.l_bound) < RDL_FACTOR*RDL):
	...
	if val < 2*SIG_MULT*SIG_D and (Cl2.ltid != Cl2.rtid or (rPos >= DiscordCluster.l_bound and lPos <= DiscordCluster.r_bound)):
            return 1
            
     if map_type == 1 and (iPos - DiscordCluster.l_bound_O) < CLUSTER_D + .5*SIG_MULT*SIG_D:
        return 1
...
def breakLeft(cluster, READ_BOUND1):
	broken = 0
	cluster2 = -1
	for g,item in enumerate(cluster.readList):
                        if g > 0 and item.l_read_bound - prev_l_read > READ_BOUND1:
	...
	return broken, cluster2

def breakRight(cluster, READ_BOUND1):
...
def writeClusters(CList, fp, fp2, READ_BOUND1):
	...
	for k,cluster in enumerate(CList):
		...
		while l_broken or r_broken:
			l_broken = 0
			r_broken = 0
			#cluster_copy = copy.deepcopy(cluster)
			if cluster != -1 and cluster.count > MIN_CLUSTER_SIZE:
				[l_broken, cluster2] = breakLeft(cluster, READ_BOUND1)	
			if cluster != -1 and cluster.count > MIN_CLUSTER_SIZE and not l_broken:
			   cluster.readList = sorted(cluster.readList,key = lambda member: member.r_read_bound)
			   print "Cluster is", cluster
			   [r_broken, cluster2] = breakRight(cluster, READ_BOUND1)
			if cluster.count > MIN_CLUSTER_SIZE:
				brClusters.append(cluster)
			cluster = cluster2
				#brClusters.append(cluster2)
				#shift+=1
			loop_count+=1
		...
		#print "Broken clusters.", cluster.count, cluster2.count

	for h,cluster_w in enumerate(brClusters):
		calculateMargin(cluster_w)
		fp.write("%s %s\n" %(1+h,cluster_w))
		writeSupport(fp2,1+h,cluster_w.support)

	print n_breaks, "clusters were broken down."
