...
if __name__ == "__main__":
        ...
        while line2:
			...
			if CN >= 4:
                ...
                if match:
                    ...
                    # With RD more than one match is not indicated for haploid. Remove for diploid or otherwise possible conflicts.Hmm, or be slightly liberal and keep it commented.
                    #break

 	if not RDcounter in Claimed and Anymatch:
				print "New variant found.", line_cnv
				newRDVars.append(line_cnv)
    ...
    for h,line3 in enumerate(fp2):
		...
		if variantNum in homozygous:
			line_split[11] = line_split[11] + "_H"
	...
	fp3.write("%s %s %s %s %s %s %s %s %s %s %s %s %s\n" %(RDcounter+1+k, typeV, "-1", "-1", "-1", chrom, int(start)-RD_MARGIN/2, int(start)+RD_MARGIN/2, chrom, int(stop)-RD_MARGIN/2, int(stop) +RD_MARGIN/2, tag, "1"))
