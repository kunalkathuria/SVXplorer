...
if __name__=="__main__":
	...
	for line in f:
		...
		if line_split[6] == "BND":
			...
			fw.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" %(line_split[0], line_split[1],counter,".", "<DEL>",".","PASS", "SVTYPE=DEL;END="+line_split[5]+";SVLEN=-"+str(int(line_split[5])-int(line_split[1]))+";IMPRECISE"))
			...
			fw.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" %(line_split[0], line_split[2],counter,".", "<DUP:TANDEM>",".","PASS", "SVTYPE=DUP;END=" + line_split[4]+";SVLEN="+str(int(line_split[4])-int(line_split[2]))+";IMPRECISE;"+isinv))

		elif line_split[6] == "INV":

                        fw.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" %(line_split[0], line_split[1],counter,".", "<INV>",".","PASS", "SVTYPE=INV;END="+line_split[4]+";SVLEN="+str(int(line_split[4])-int(line_split[2]))+";IMPRECISE"))
			...
			if line_split[6]=="INS_I":
				isinv="ISINV"
			fw.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" %(line_split[3], line_split[4],counter,".", "<DUP:ISP>",".","PASS", "SVTYPE=DUP;CHR2="+line_split[0]+";POS2="+line_split[1]+";END2="+line_split[2] +";SVLEN="+str(int(line_split[2])-int(line_split[1])) +";IMPRECISE;"+isinv))
		...
		#$for INS_C write separately with comment of bp may be switched, provide alternate	
			...
			len_tr = int(line_split[2]) - int(line_split[1])
                        fw.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" %(line_split[0], line_split[1],counter,".", "<DEL:TRA>",".","PASS", "SVTYPE=DEL;TRAID="+str(counter)+";SVLEN=-" + str(len_tr) + ";IMPRECISE;"+ isinv))
                        fw.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" %(line_split[3], line_split[4],counter+1,".", "<DUP:TRA>",".","PASS", "SVTYPE=DUP;TRAID="+str(counter) + ";SVLEN=" + str(len_tr) + ";CHR2=" + line_split[0] + ";POS2=" + line_split[1] + ";END2=" + line_split[2] + ";IMPRECISE;"+isinv))
			counter+=1
