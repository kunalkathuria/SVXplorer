...
def compareCluster(cluster1, Clusters, Claimed, graph_c, graph_m, offset, LR, OCArray):
    ...
    for x, item in enumerate(Clusters):
        ...
        Cl1_small = int(cluster1.clsmall)
	Cl2_small = int(Clusters[x].clsmall)
        
	# flanking cluster for all small SVs 
##	if cluster1.ltid == cluster1.rtid and cluster1.l_orient == 0 and cluster1.r_orient == 1 and cluster1.r_end - cluster1.l_start < MEAN_IL - SIG_MULT_RD*SIG_IL:
##		Cl1_small = 1
##	if Clusters[x].ltid == Clusters[x].rtid and Clusters[x].l_orient == 0 and Clusters[x].r_orient == 1 and Clusters[x].r_end - Clusters[x].l_start < MEAN_IL - SIG_MULT_RD*SIG_IL:
##		Cl2_small = 1
       
        #$ Add checks to see if at least 2 clusters have ALL TIDs same for INS -- DONE

        if cluster1.r_orient == 2 and Clusters[x].r_orient == 2 and sign1 and cluster1.l_orient != Clusters[x].l_orient:
        ...
        elif sign1 and sign4 and cluster1.l_orient != Clusters[x].l_orient and cluster1.r_orient != Clusters[x].r_orient and cluster1.r_orient == cluster1.l_orient and cluster1.ltid == cluster1.rtid:
        ...
        # small insertions
            ...
            #print "Cl 1 small --2,4"

##            if Clusters[x].r_orient == 0 and not (cluster1.l_start < Clusters[x].r_start < cluster1.r_end):
##                continue
##            if Clusters[x].r_orient == 1 and not (cluster1.l_start < Clusters[x].r_end < cluster1.r_end):
##                continue
        ...
        # Large insertions
          ...
          if cluster1.ltid == Clusters[x].ltid or cluster1.ltid == cluster1.rtid or Clusters[x].ltid == cluster1.rtid:
        ...
        elif sign2 and not sign3 and (cluster1.rtid == cluster1.ltid or Clusters[x].ltid == cluster1.ltid or cluster1.rtid == Clusters[x].ltid) and cluster1.l_orient != Clusters[x].r_orient and cluster1.r_orient != Clusters[x].l_orient:
            ...
            if (cluster1.l_orient == cluster1.r_orient and Clusters[x].l_orient != Clusters[x].r_orient) or (Clusters[x].r_orient == Clusters[x].l_orient and cluster1.l_orient != cluster1.r_orient):
                type_O = "INS_C_I"
            newCl=1

            #regular insertion if both mates show inversion
            if bp_1_tid != bp_2_tid and bp_1_tid != bp_3_tid and cluster1.l_orient == cluster1.r_orient and Clusters[x].r_orient == Clusters[x].l_orient:
                    type_O = "INS_I"
        ...
        elif sign3 and not sign2 and (cluster1.rtid == cluster1.ltid or Clusters[x].rtid == cluster1.rtid or cluster1.ltid == Clusters[x].rtid) and cluster1.r_orient != Clusters[x].l_orient and cluster1.l_orient != Clusters[x].r_orient:
            ...
            # one mate same orientation, other opposite, then cut insertion
            if (cluster1.l_orient == cluster1.r_orient and Clusters[x].l_orient != Clusters[x].r_orient) or (Clusters[x].r_orient == Clusters[x].l_orient and cluster1.l_orient != cluster1.r_orient):
                type_O = "INS_C_I"
            newCl=1
...
##            graph_c.add_edge(cl1,connection,1)
        ...
        if elem.typeO == "TD_I":
            ...
            if cloc1_tid == elem.bp1tid and cloc2_tid == elem.bp3tid and elem.bp1_orient != -1 and elem.bp3_orient != -1 and isOverlapping("V",cluster1,elem,"L1") and cluster1.l_orient != elem.bp1_orient and isOverlapping("V",cluster1,elem,"R3") and cluster1.r_orient != elem.bp3_orient:
            ...
            elif cloc1_tid == elem.bp3tid and cloc2_tid == elem.bp1tid and elem.bp1_orient != -1 and elem.bp3_orient != -1 and isOverlapping("V",cluster1,elem,"R1") and cluster1.r_orient != elem.bp1_orient and isOverlapping("V",cluster1,elem,"L3") and cluster1.l_orient != elem.bp3_orient:
            ...
            elif (elem.typeO == "INS_C_P" or elem.typeO == "INS_C_I_P") and cloc1_tid == elem.bp3tid and cloc2_tid == elem.bp1tid and elem.bp1_orient != -1 and elem.bp2_orient != -1 and isOverlapping("V",cluster1,elem,"L3") and cluster1.l_orient != elem.bp3_orient and isOverlapping("V",cluster1,elem,"R1") and cluster1.r_orient != elem.bp1_orient:       
                ...
                elem.bp1_orient = -1
                elem.bp3_orient = -1
                print "INS_C_P 1", elem.count

            elif (elem.typeO == "INS_C_P" or elem.typeO == "INS_C_I_P") and cloc1_tid == elem.bp1tid and cloc2_tid == elem.bp3tid and elem.bp1_orient != -1 and elem.bp2_orient != -1 and isOverlapping("V",cluster1,elem,"L1") and cluster1.l_orient != elem.bp1_orient and isOverlapping("V",cluster1,elem,"R3") and cluster1.r_orient != elem.bp3_orient:
                ...
                elem.bp1_orient = -1
                elem.bp3_orient = -1
                print "INS_C_P 2", elem.count
                
            # small or medium insertions, $ can add cl_small in this condition (not done to be liberal in case small flag won't be set for small INS)
            ...
            elif not (elem.typeO == "INS_C_P" or elem.typeO == "INS_C_I_P") and isOverlapping("V", cluster1, elem, "L3") and cloc1_tid == elem.bp3tid and isOverlapping("V", cluster1, elem, "R3") and cloc2_tid == elem.bp3tid and cluster1.l_orient != cluster1.r_orient:
                    ...
                    if elem.bp1tid == elem.bp2tid:
                        if elem.typeO == "INS_I" and cluster1.l_orient == cluster1.r_orient:
                            ...
                            if elem.bp2_start < elem.bp1_start:
                                #swap 1 and 2: hmm, not sure why this condition would ever occur but no harm
                                ...
                                #swap 1 and 3 because 1 is the right pasted location
                                bpList = [elem.bp1_start, elem.bp1_end, elem.bp3_start, elem.bp3_end]
                                tidList = [elem.bp1tid, elem.bp3tid]
                                swapBPs(bpList, tidList)
                                [elem.bp1_start, elem.bp1_end, elem.bp3_start, elem.bp3_end] = bpList
...
if __name__ == "__main__":
    ...
    #fp6 = open("../results/text/All_Clusters_I.txt","r")
...
##        if len(Clusters_L) > 0:
##		
##	 	if tempL.ltid != Clusters_L[-1].ltid:
##			
##			#print "New TID: Processing ltid", temp.ltid, line, "Size of list:", len(DList)
##			newBlockS = 0
## 	                Clusters_L = []
##                        
##		elif temp.ltid == Clusters_L[newBlockS].ltid and (tempL.l_start - Clusters_L[newBlockS].l_start) > MARGIN:
##
##
##                    Clusters_L = Clusters_L[newBlockS:]
##                    newBlockS = len(Clusters_L)
##	
##        compareCluster(tempL, Clusters_L, Claimed, compareGraph, matchGraph, offset, "LL", OCArray)
##        Clusters_L.append(tempL)
    ...
    for line in fpL:
	    ...
	    #elif cluster.l_start < cluster.r_start and cluster.l_orient == cluster.r_orient and cluster.ltid == cluster.rtid:    
		#temp.typeO = "INV"
