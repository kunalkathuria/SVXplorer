	...
	for line2 in f1:
		...
		if y[num] > 0:
			...
			if line2_split[11].find("RD") == -1: 
				...
				elif False and len(line2_split[1]) > 4 and line2_split[1][0:4] == "INS_C" and line_split[8] != "-1":
					if line2_split[2] == line2_split[5] and not (int(line2_split[6]) < int(line2_split[3]) < int(line2_split[10])): #and covLoc/COVERAGE > DUP_THRESH:
						...
						if counter2 > MIN_PILEUP_THRESH and (counter2 > GOOD_REG_THRESH*(stop-start) or counter2 > PILEUP_THRESH):
                                                        ...
                                                        if covLoc/COVERAGE < DEL_THRESH:
                                                                line2_split[1] = "DEL"
                                                                #write deletion
                                                                f13.write(("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n") %(line2_split[2], start -.25*gap - (int(line2_split[4]) - int(line_split[3])), start-.25*gap, line2_split[2], start+.75*gap, start+.75*gap + int(line2_split[4]) - int(line_split[3]),"DEL",GT))
                                                                ...
                                                                if counter_l > MIN_PILEUP_THRESH and (counter_l > GOOD_REG_THRESH*(stop-start) or counter_l > PILEUP_THRESH):
                                                                        ...
                                                                        if covLoc_l/COVERAGE >= 1.0:
                                                                                line2_split[1] = "TD"
                                                                                #write TD
                                                                                f13.write(("%s\t%s\t%s\t%s\t%s\t%s\t%s\n") %(line2_split[2], start2 -.25*gap2 - (int(line2_split[4]) - int(line_split[3])), start2-.25*gap2, line2_split[2], start2+.75*gap2, start2+.75*gap2 + int(line2_split[4]) - int(line_split[3]),"TD"))
                                                                continue
