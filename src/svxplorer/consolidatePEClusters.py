...
class clusterI(object):
    ...
    def __str__(self):
        return "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
            self.mapNum, self.l_start, self.l_end, self.r_start, self.r_end, self.typeC, self.lTID, self.rTID,
            self.l_orient, self.r_orient)
...
class consCluster(object):
    ...
    def __str__(self):
        return "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
            self.SVType, self.bp1TID, self.bp1_start, self.bp1_end, self.bp2TID, self.bp2_start, self.bp2_end,
            self.bp3TID,
            self.bp3_start, self.bp3_end, self.support, self.count)
...
def compareCluster(cluster1, clusters, claimedCls, consolidatedCls,
    ...
    for clusterP in clusters:
        ...
        if cluster1.mapNum == clusterP.mapNum or (dontCompareSet is not None and \
                                                  ((cluster1.mapNum, clusterP.mapNum) in dontCompareSet or (
                                                          clusterP.mapNum, cluster1.mapNum) in dontCompareSet)):
        ...
        if cluster1.r_orient != 2 and clusterP.r_orient != 2:
            ...
            # Large insertions
                ...
                if newVariant.bp1TID != newVariant.bp2TID and newVariant.bp1TID != newVariant.bp3TID \
                    ...
                    logging.debug(
                        'Large INS check 3: mark as cut-paste if one of source breakpoints is on same chr as overlap pt')
                ...
                if clusterP.lTID == clusterP.rTID and not (cluster1.r_end < clusterP.r_end - RDL_Factor * RDL):
                    logging.debug('Not safe distance between overlap point and other 2 mate almts:2 %d %d',
                                  cluster1.r_end, clusterP.r_end)
                    continue
                ...
                if newVariant.bp1TID != newVariant.bp2TID and newVariant.bp1TID != newVariant.bp3TID \
                    ...
                    logging.debug(
                        'Large INS check 4: mark as cut-paste if one of source breakpoints is on same chr as overlap pt')
                    if newVariant.bp1TID == newVariant.bp2TID and newVariant.bp3TID != newVariant.bp1TID:
                        ...
                        if newVariant.SVType == "INS_C":
                            newVariant.SVType = "INS_C_P"
                        logging.debug('%d %d %d %d', newVariant.bp1_start, newVariant.bp1_end, newVariant.bp2_start,
                                      newVariant.bp2_end)
                        # Put 2 and 3 on same chromosome, as 1 is pasted location in our convention
                        newVariant.bp1_start, newVariant.bp1_end, newVariant.bp2_start, newVariant.bp2_end = \
                            newVariant.bp2_start, newVariant.bp2_end, newVariant.bp1_start, newVariant.bp1_end
...
def compareVariant(cluster1, varList, claimedCls, slop, as_relative_thresh,
    ...
    for g, elem in enumerate(varList):
        ...
        if elem.SVType == "DN_INS" and (cluster1.isSmall and cluster1.lTID == elem.bp1TID and \
                                        (isOverlapping("V", cluster1, elem, "L1", slop) or isOverlapping("V", cluster1,
                                                                                                         elem, "R1",
                                                                                                         slop)) or \
                                        (cluster1.r_orient == 2 and cluster1.lTID == elem.bp1TID and isOverlapping("V",
                                                                                                                   cluster1,
                                                                                                                   elem,
                                                                                                                   "L1",
                                                                                                                   slop))):
        ...
        if cluster1.r_orient != 2:
            ...
            if elem.SVType == "TD_I":
                ...
                if cluster1.isSmall and cluster1.lTID == elem.bp2TID and cluster1.rTID == elem.bp2TID and \
                        isOverlapping("V", cluster1, elem, "L2", slop) and isOverlapping("V", cluster1, elem, "R2",
                                                                                         slop):
                ...
                # if cluster indicating translocation deletion arrives here
                        ...
                        isOverlapping("V", cluster1, elem, "L2", slop) and not isOverlapping("V", cluster1, elem, "R1",
                                                                                             slop) \
                    ...
                    elem.SVType = "INS_C"
                        ...
                        not isOverlapping("V", cluster1, elem, "L1", slop) and isOverlapping("V", cluster1, elem, "R2",
                                                                                             slop) \
                ...
                if cluster1.lTID == elem.bp1TID and cluster1.rTID == elem.bp3TID and elem.bp1_orient != -1 \
                    ...
                    elem.bp1_orient = -1
                    elem.bp3_orient = -1
                    elem.bp1_start, elem.bp1_end = sorted(
                        [elem.bp1_start, elem.bp1_end, cluster1.l_start, cluster1.l_end])[1:3]
                    elem.bp3_start, elem.bp3_end = sorted(
                        [elem.bp3_start, elem.bp3_end, cluster1.r_start, cluster1.r_end])[1:3]
                    ...
                    elem.bp3_orient = -1
                    elem.bp1_start, elem.bp1_end = sorted(
                        [elem.bp1_start, elem.bp1_end, cluster1.r_start, cluster1.r_end])[1:3]
                    elem.bp3_start, elem.bp3_end = sorted(
                        [elem.bp3_start, elem.bp3_end, cluster1.l_start, cluster1.l_end])[1:3]
                    ...
                    elem.bp1_start, elem.bp1_end = sorted(
                        [elem.bp1_start, elem.bp1_end, cluster1.l_start, cluster1.l_end])[1:3]
                    elem.bp2_start, elem.bp2_end = sorted(
                        [elem.bp2_start, elem.bp2_end, cluster1.r_start, cluster1.r_end])[1:3]
                    # bp has 2 overlapping reads now
                    # print "INS_C 1", elem.count
                    ...
                    elem.bp1_start, elem.bp1_end = sorted(
                        [elem.bp1_start, elem.bp1_end, cluster1.r_start, cluster1.r_end])[1:3]
                    elem.bp2_start, elem.bp2_end = sorted(
                        [elem.bp2_start, elem.bp2_end, cluster1.l_start, cluster1.l_end])[1:3]
                ...
                if cluster1.isSmall and (isOverlapping("V", cluster1, elem, "L1", slop) and \
                                         cluster1.lTID == elem.bp1TID) and (
                        isOverlapping("V", cluster1, elem, "R2", slop) and \
                ...
                # small cluster check
                        ...
                        isOverlapping("V", cluster1, elem, "L1", slop) and isOverlapping("V", cluster1, elem, "R1",
                                                                                         slop):
...
def consolidatePEClusters(workDir, statFile, clusterFile,
    ...
    fVariantsPE.write(
        "VariantNum\tType\tchr1\tstart1\tstop1\tchr2\tstart2\tstop2\tchr3\tstart3\tstop3\tSupportBy\tNPEClusterSupp\tNFragPESupp\tNFragSRSupp\tOrientation\n")
    hashedVM = {}
...
def main():
    ...
    PARSER.add_argument('clusterMapFile',
                        help='File containing list of clusters and their supporting fragments, typically clusterMap.txt')
    ...
    consolidatePEClusters(ARGS.workDir, ARGS.statFile, ARGS.clusterFile, ARGS.clusterMapFile, ARGS.slop,
                          ARGS.as_relative_thresh, False)
