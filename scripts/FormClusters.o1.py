...
def CompareCluster(map_type, DiscordCluster, Cl2):
   ...
   if Cl2.C_type == DiscordCluster.C_type and Cl2.ltid == DiscordCluster.ltid and Cl2.rtid == DiscordCluster.rtid and DiscordCluster.clsmall == Cl2.clsmall:
 
     if map_type == 0 and ((lPos - DiscordCluster.l_bound_O) < CLUSTER_D) or abs(lPos-DiscordCluster.l_bound) < RDL_FACTOR*RDL:
	...
	if val < 2*DISC_D and (Cl2.ltid != Cl2.rtid or (rPos >= DiscordCluster.l_bound and lPos <= DiscordCluster.r_bound)):
     ...
     if map_type == 1 and ((iPos - DiscordCluster.l_bound_O) < CLUSTER_D or abs(lPos-DiscordCluster.l_bound) < RDL_FACTOR*RDL):
        return 1
...
def calculateMarginI(item):

                l_orient = int(item.C_type[0])

                cl_margin = MEAN_D + DISC_D - 2*RDL - (item.lmax - item.lmin)
...
def calculateMargin(item):
		...
		cl_margin_l = MEAN_D + DISC_D - 2*RDL - (item.lmax - item.lmin)
		cl_margin_r = MEAN_D + DISC_D - 2*RDL - (item.rmax - item.rmin)
		...
		if item.ltid == item.rtid: 
			if not l_orient and r_orient:
				...
				item.l_end = min(item.l_end, item.r_end)
				item.r_start = max(item.r_start, item.l_start)

			elif not l_orient and not r_orient:

                                item.l_end = min(item.l_end, item.r_start)

			elif l_orient and r_orient:

				item.r_start = max(item.r_start, item.l_end)
...
def writeClustersI(CList, fp, fp2, READ_BOUND1):
	...
	shift = 0
        ...
        for k,cluster in enumerate(CList):
                ...
                #check if left reads have a gap somewhere to merit breaking cluster
...
def writeClusters(CList, fp, fp2, READ_BOUND1):
	...
	for k,cluster in enumerate(CList):
		...
		#see if right reads have large-enough gap to merit cluster breaking	
		if not broken:
...
if __name__== "__main__":
	...
	if temp.l_bound > temp.r_bound and temp.ltid == temp.rtid and temp.C_type == "01":
		swap(l_orient, r_orient)
		swap(temp.lmin, temp.rmin)
		swap(temp.lmax, temp.rmax)
		temp.C_type = "10"
    ...
    #single mappings from PE/unique insertions
    clusterList = []
    DList = []
    counter = 0
    newBlockS = 0
	...
	ld = len(DList)
        ...
        for x in range(ld):
            ...
            ClMatch = CompareCluster(1, DList[ld-x-1], temp)
    ...
    ld1=ld1+offset+len(DList)
    f2.close()

    print ld1, " total clusters."
    print "clusters in total (but not all necessarily significant/listed)."

    #may get better statistics on using previous, as likely larger sample
    #[RB1, RB2] = calcReadStats(clusterList)
    writeClustersI(clusterList, f3, f4, RB1)
