...
class cluster(object):
    ...
    def __str__(self):
        return "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
            self.count, self.cType, self.lTID, self.l_start, self.l_end, self.rTID, self.r_start, self.r_end,
            self.clSmall)
...
def calcEdgeWeight(f1_lPos, f1_rPos, f2_lPos, f2_rPos, IL_BinTotalEntries, c_type, dist_penalty, dist_end, rdl,
                   IL_BinDistHash, mean_IL, isSmall):
...
def processNewFrag(fragList, almt, IL_BinTotalEntries, fragmentGraph,
    ...
    for storedAlmt in fragList:
        if storedAlmt.lTID == almt.lTID and \
            ...
            logging.debug("Comparing %s~%s-%s to %s~%s-%s", almt.fragNum, almt.l_bound, almt.r_bound,
                          storedAlmt.fragNum, storedAlmt.l_bound, storedAlmt.r_bound)
            # compare 2 frags
            ...
            edge_weight = calcEdgeWeight(f1_lPos, f1_rPos, f2_lPos, f2_rPos, IL_BinTotalEntries, almt.cType,
                                         dist_penalty, dist_end, rdl, IL_BinDistHash, mean_IL, almt.clSmall)
...
def refreshFragList(fragList, almt, fragmentGraph, fragHash,
    ...
    if len(fragList) > 1:
        if almt.lTID != fragList[-2].lTID or almt.rTID != fragList[-2].rTID:
            ...
            clusterNum = writeClusters(fragmentGraph, fragHash, fCliques, fClusters, fClusterMap, [], clusterNum,
                                       mean_IL, disc_thresh, max_cluster_length, bp_margin, min_cluster_size, debug)
            fragmentGraph.clear()
            ...
            clusterNum = writeClusters(fragmentGraph, fragHash, fCliques, fClusters, fClusterMap, nodeList, clusterNum,
                                       mean_IL, disc_thresh, max_cluster_length, bp_margin, min_cluster_size, debug)
    ...
    for line_num, line in enumerate(fDiscAlmts):
        ...
        if subsample:
            ...
            if found and not process:
                logging.debug('Subsampling %s:%s - %s:%s since the coverage is extremely high', almt.lTID, almt.l_bound,
                              almt.rTID, almt.r_bound)
                continue
            elif not found:
        ...
        processNewFrag(fragList, almt, IL_BinTotalEntries, fragmentGraph, edge_weight_thresh, dist_penalty, dist_end,
                       rdl, IL_BinDistHash, mean_IL, debug)

        # refresh fragment list
        fragList, clusterNum, newClusterBlock, fragHash = refreshFragList(fragList, almt, fragmentGraph, fragHash,
                                                                          fCliques, fClusters, fClusterMap,
                                                                          max_cluster_length, clusterNum,
                                                                          newClusterBlock, mean_IL, disc_thresh,
                                                                          bp_margin, debug, min_cluster_size)
    logging.info('Finished PE cluster formation')

    logging.info('Started writing remaining clusters')
    clusterNum = writeClusters(fragmentGraph, fragHash, fCliques, fClusters, fClusterMap, [], clusterNum, mean_IL,
                               disc_thresh, max_cluster_length, bp_margin, min_cluster_size, debug)
