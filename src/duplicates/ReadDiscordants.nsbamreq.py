...
CALC_THRESH = 2*int(sys.argv[6]) #500000 # boost arbitraily due to filters for both mates 
...
def calcMeanSig(file1, file2):
   ...
   while counter < CALC_THRESH and loopCount < 2*CALC_THRESH:
	...
	# Use only unsplit alignments to estimate largest outer distance
        if m1.query_name == m2.query_name and m1.is_proper_pair and m2.is_proper_pair and not m1.is_secondary and not m1.is_supplementary and not m2.is_secondary and not m2.is_supplementary and m1.get_tag("AS") > AS_CALC_THRESH*m1.infer_query_length() and m2.get_tag("AS") > AS_CALC_THRESH*m2.infer_query_length():
   ...
   #generalized 3 sigma if not normal distribution, SIG_THRESH = 99.85
   ...
   while counter < CALC_THRESH and loopCount < 2*CALC_THRESH:
        ...
        if m1.query_name == m2.query_name and m1.is_proper_pair and m2.is_proper_pair and not m1.is_secondary and not m1.is_supplementary and not m2.is_secondary and not m2.is_supplementary and m1.get_tag("AS") > AS_CALC_THRESH*m1.infer_query_length() and m2.get_tag("AS") > AS_CALC_THRESH*m2.infer_query_length():
	     ...
	     stdev+= (abs(m1.template_length) - meanIL)**2
	     counter+=1
	loopCount+=1
