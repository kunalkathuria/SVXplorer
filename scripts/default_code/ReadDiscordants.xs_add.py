...
def findTotalNMatches(al):
	...
	if n_matches > 0:
		return float(n_matches)/float(n_matches + n_subs)
	else:
		return 0
...
def FormDiscordant(list1, list2, DList1, DList2):
    ...
    for al1 in list1:
		...
		if ASF1 < AS_THRESH*al1.infer_query_length():
			continue
		if ASF2 < AS_THRESH*al2.infer_query_length():
                        continue
		...
		if RMatchF2 > 0:
			MatchR_2 = float(RMatch2)/float(RMatchF2)
                ...
                if RMatch1 > MATCH_PCT and MatchR_1 >= MATCH_THRESH and Al_S_1 >= PCT_THRESH*ASF1:
			al1_match = 1

                else:
                    continue # can use return here if sure BAM file reports alignments in order of decreasing AS
		
                if RMatch2 > MATCH_PCT and MatchR_2 >= MATCH_THRESH and Al_S_2 >= PCT_THRESH*ASF2:
                    	al2_match = 1
