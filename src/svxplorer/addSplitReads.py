...
import logging
import gc
from utils import formExcludeHash, ignoreRead, readChromosomeLengths, countLines
...
def addSplitReads(workDir, variantMapFilePE, allVariantFilePE, bamFileSR,
    ...
    while True:
        ...
        if sr1.qname == sr2.qname:
            ...
            if sr1.reference_name in ignoreList or \
                    ...
                    (sr_bp1_tid == bp1TID and sr_bp2_tid == bp2TID and abs(sr_bp1 - bp1Prev) < refRate and abs(
                        sr_bp2 - bp2Prev) < refRate):
                continue
        ...
        # check INV
            ...
            if (SVHashPE[
                    searchAlmt].orient == "00" and minsr.reference_end > sr_bp1 and maxsr.reference_end > sr_bp2) or \
                    (SVHashPE[
                         searchAlmt].orient == "11" and minsr.reference_start < sr_bp1 and maxsr.reference_start < sr_bp2):
                newBp[3] = 1
                # check INS
            ...
            if varNumPE in SRtoPESuppBPs and (SRtoPESuppBPs[varNumPE][2] == -1 \
                ...
                if varType in [3, 4] and SRtoPESuppBPs[varNumPE][1] != -1 \
                        and SRtoPESuppBPs[varNumPE][2] != -1 and SRtoPESuppBPs[varNumPE][2] < SRtoPESuppBPs[varNumPE][
                    1]:
            ...
            if almtMatchesSVbps == 1:
                ...
                if SRVarHash[newAlmt].typeSV == "DEL_INS" or SRVarHash[newAlmt].typeSV == "DEL":
                    if l_orient == r_orient and swap == 1 and not (SRVarHash[newAlmt].bp2 - slop / 2 <= other_bp \
                                                                   <= SRVarHash[newAlmt].bp2 + slop / 2) and not (
                            SRVarHash[newAlmt].bp2 < newAlmt[2] < other_bp or \
                    ...
                    elif swap == 0 and SRVarHash[newAlmt].bp2 - slop / 2 <= other_bp <= SRVarHash[
                        newAlmt].bp2 + slop / 2 \
                ...
                # TD_I
                    ...
                    if swap == 1 and l_orient == r_orient and SRVarHash[newAlmt].bp2 - slop / 2 <= other_bp \
                        ...
                        # print "TD 1", SRVarHash[newAlmt].count
                                                                     ...
                                                                     <= SRVarHash[newAlmt].bp2 + slop / 2) and not (
                            other_bp < newAlmt[2] < SRVarHash[newAlmt].bp2 or \
                    ...
                    if SRVarHash[newAlmt].bp2 - slop / 2 <= other_bp <= SRVarHash[newAlmt].bp2 + slop / 2:
                        SRVarHash[newAlmt].count += 1
                        SRVarHash[newAlmt].support.append(SRFrag)
                    elif detectIntraChrCopyInv and (
                            SRVarHash[newAlmt].bp2 - slop / 2 <= other_bp <= SRVarHash[newAlmt].bp2 + slop / 2 or \
                            SRVarHash[newAlmt].bp2 < newAlmt[2] < other_bp or other_bp < newAlmt[2] < SRVarHash[
                                newAlmt].bp2):
                    ...
                    if l_orient == r_orient and (SRVarHash[newAlmt].bp3 == -1 or SRVarHash[newAlmt].bp2 - slop / 2 \
                                                 <= other_bp <= SRVarHash[newAlmt].bp2 + slop / 2 or SRVarHash[
                                                     newAlmt].bp3 - slop / 2 \
                        ...
                        if SRVarHash[newAlmt].bp3 == -1 and not (SRVarHash[newAlmt].bp2 - slop / 2 <= other_bp \
                                                                 <= SRVarHash[newAlmt].bp2 + slop / 2 or SRVarHash[
                                                                     newAlmt].bp2 < newAlmt[2] < other_bp \
    ...
    fAVN.write(
        "VariantNum\tType\tchr1\tstart1\tstop1\tchr2\tstart2\tstop2\tchr3\tstart3\tstop3\tSupportBy\tNPEClusterSupp\tNFragPESupp\tNFragSRSupp\n")
    fAV.seek(0)
    header = fAV.readline()
    for lineVM in fVM:
...
def main():
    PARSER = argparse.ArgumentParser(
        description='Add split reads to support existing PE variants and create new SR variants')
    ...
    addSplitReads(ARGS.workDir, ARGS.variantMapFilePE, ARGS.allVariantFilePE,
                  ...
                  ARGS.minSRtoPEsupport, ARGS.ignoreBED, ARGS.x)  # TODO: maxClusterMargin arg not filled!
