#KK
#Accepts list of discordant alignemnts with attributes from ReadDiscordant.py output and forms/writes out clusters based on weighted clique algorithm
...
DISC_ENHANCER = 1.67 
# 1.67 default, wil multiply "3 sigma" (99.85 %tile) disc distance by a "safety" factor to not miss larger sampling IL clusters
...
def writeClusters(G, fragHash, fc, ft, f3, f4):
	...
	if len(connected_comp) >= MIN_CLUSTER_SIZE:
		...
		edgeC_T = 0
		maxavgW = 0
		for k,cl in enumerate(currentCliques):
			sumW = 0
			edgeC = 0
			for item in cl:
...
if __name__== "__main__":
    ...
    f1=open("./results/All_Discords_P_S.txt","r")
    f3=open("./results/All_Clusters.txt","w")
    f4=open("./results/VariantMap.txt","w")
    fc = open("./results/cluster_cliques.txt","w")
    ft=open("./results/cc_size.txt","w") 
