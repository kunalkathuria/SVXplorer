...
DISC_ENHANCER2 = DISC_ENHANCER*(5.0/5.0) # safety factor for cluster margin (e.g. if normal distr, 3 sig for disc marking, 4 sig for cluster margin, 5 sig for cluster inclusion)
...
LIB_MULT = int(sys.argv[10]) #liberal distance multiplier to include reads in given cluster even if far from beginning of cluster; used if reads piling up close; cluster will be broken later based on density
...
def calculateMargin(item):
		...
		cl_margin_l = MEAN_D + 1.0*DISC_D - 2*RDL - (item.lmax - item.lmin)
		cl_margin_r = MEAN_D + 1.0*DISC_D - 2*RDL - (item.rmax - item.rmin)
		...
		if item.l_start < 0:
                ...
                if item.ltid == item.rtid:
                        if not l_orient and r_orient:
                                ...
                                item.r_start = max(item.l_max+1, item.r_start)

                        elif l_orient and r_orient:

                                item.l_end = min(item.r_min-1, item.l_end)
