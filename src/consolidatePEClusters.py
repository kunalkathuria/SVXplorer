...
def isOverlapping(CorV, clus1, clus2, LR, slop):
    if CorV == "C":
        if LR=="LL":
            if (clus2.l_start >= clus1.l_start and clus2.l_start <= clus1.l_end) \
                    or (clus2.l_end >= clus1.l_start and clus2.l_end <= clus1.l_end) \
                    or abs(clus2.l_start - clus1.l_end) < slop or abs(clus1.l_start - clus2.l_end) < slop:
                return 1
        elif LR=="LR":
            if (clus2.l_start >= clus1.r_start and clus2.l_start <= clus1.r_end) \
                    or (clus2.l_end >= clus1.r_start and clus2.l_end <= clus1.r_end) \
                    or abs(clus2.l_start - clus1.r_end) < slop or abs(clus1.r_start - clus2.l_end) < slop:
                return 1
        elif LR=="RL":
            if (clus2.r_start >= clus1.l_start and clus2.r_start <= clus1.l_end) \
                    or (clus2.r_end >= clus1.l_start and clus2.r_end <= clus1.l_end) \
                    or abs(clus2.r_start - clus1.l_end) < slop or abs(clus1.l_start - clus2.r_end) < slop:
                return 1
        elif LR=="RR":
            if (clus2.r_start >= clus1.r_start and clus2.r_start <= clus1.r_end) \
                    or (clus2.r_end >= clus1.r_start and clus2.r_end <= clus1.r_end) \
                    or abs(clus2.r_start - clus1.r_end) < slop or abs(clus1.r_start - clus2.r_end) < slop:
                return 1
        return 0

    elif CorV == "V":
        if LR=="L1":
           if (clus1.l_start >= clus2.bp1_start and clus1.l_start <= clus2.bp1_end) \
                or (clus1.l_end >= clus2.bp1_start and clus1.l_end <= clus2.bp1_end) \
                or abs(clus1.l_start - clus2.bp1_end) < slop or abs(clus2.bp1_start - clus1.l_end) < slop:
                return 1
        elif LR=="L2":
            if (clus1.l_start >= clus2.bp2_start and clus1.l_start <= clus2.bp2_end) \
                or (clus1.l_end >= clus2.bp2_start and clus1.l_end <= clus2.bp2_end) \
                or abs(clus1.l_start - clus2.bp2_end) < slop or abs(clus2.bp2_start - clus1.l_end) < slop:
                return 1
        elif LR=="L3":
            if (clus1.l_start >= clus2.bp3_start and clus1.l_start <= clus2.bp3_end) \
                or (clus1.l_end >= clus2.bp3_start and clus1.l_end <= clus2.bp3_end) \
                or abs(clus1.l_start - clus2.bp3_end) < slop or abs(clus2.bp3_start - clus1.l_end) < slop:
                return 1
        elif LR=="R1":
            if (clus1.r_start >= clus2.bp1_start and clus1.r_start <= clus2.bp1_end) \
                or (clus1.r_end >= clus2.bp1_start and clus1.r_end <= clus2.bp1_end) \
                or abs(clus1.r_start - clus2.bp1_end) < slop or abs(clus2.bp1_start - clus1.r_end) < slop:
                return 1
        elif LR=="R2":
            if (clus1.r_start >= clus2.bp2_start and clus1.r_start <= clus2.bp2_end) \
                or (clus1.r_end >= clus2.bp2_start and clus1.r_end <= clus2.bp2_end) \
                or abs(clus1.r_start - clus2.bp2_end) < slop or abs(clus2.bp2_start - clus1.r_end) < slop:
                return 1
        elif LR=="R3":
            if (clus1.r_start >= clus2.bp3_start and clus1.r_start <= clus2.bp3_end) \
                or (clus1.r_end >= clus2.bp3_start and clus1.r_end <= clus2.bp3_end) \
                or abs(clus1.r_start - clus2.bp3_end) < slop or abs(clus2.bp3_start - clus1.r_end) < slop:
                return 1
...
def compareCluster(cluster1, clusters, claimedCls, graph_c, graph_m, LR, consolidatedCls, slop):
    anyMatch = 0 
    ...
    for clusterP in clusters:
        ...
        # Large insertions
            ...
            if newVariant.SVType == "INS" or newVariant.SVType == "INS_I":
                ...
                # check for cut-paste
                formCutPasteINS(newVariant, cluster1, clusterP, "LL")
          ...
          if newVariant.SVType == "INS" or newVariant.SVType == "INS_I":
              ...
              # check for cut-paste
              formCutPasteINS(newVariant, cluster1, clusterP, "RR")
            ...
            cluster1.l_orient != clusterP.r_orient and cluster1.r_orient != clusterP.l_orient:
            ...
            cluster1.r_orient != clusterP.l_orient and cluster1.l_orient != clusterP.r_orient:
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser.add_argument('clusterFileRS', help='File containing clusters sorted by right chr and position, \
            typically allClusters.rs.txt')
        parser.add_argument('clusterMapFile', help='File containing list of clusters and their supporting fragments, \
        ...
        parser.add_argument('-s', default=0,dest='slop', type=int,\
            help='Additional slop added to dynamically calculated cluster breakpoint \
            margins, if desired. Default recommended.')
        parser.add_argument('-f', default=5, dest='refRate', type=int,\
            help='Parameter controlling size of buffer variant list in cluster \
            consolidation, optimized for speed.')
    ...
    clusterFileLS = args.clusterFileLS
    clusterFileRS = args.clusterFileRS
    clusterMapFile = args.clusterMapFile
    variantNum = 1
    ...
    #fp6 = open(workDir+"/allClusters.up.txt","r")
    clusters_LS = []
    clusters_RS = []
    consolidatedCls = []
    consolidatedCls_C = []
    ...
    for lineLC in fClustersLS:
        if verbose and loopCount % 100 == 0:
            print loopCount
            print len(consolidatedCls), "current variants."
            print len(clusters_LS), "current left-sorted clusters."
            print len(clusters_RS), "current right-sorted clusters."
        ...
        compareCluster(cluster_R, clusters_LS, claimedCls, compareGraph, matchGraph, "RL", \
            consolidatedCls, slop)
        compareCluster(cluster_R, clusters_RS, claimedCls, compareGraph, matchGraph, "RR", \
            consolidatedCls, slop)
    ...
    for line in fClustersLS:
        ...
        if not clusterC.mapNum in claimedCls:
            ...
            if clusterC.r_orient == "2":
                ...
                clusterC.l_start < clusterC.r_start and clusterC.lTID == clusterC.rTID:
                ...
                clusterC.l_start < clusterC.r_end and clusterC.lTID == clusterC.rTID and \
                clusterC.isSmall != 1:
                newSimpleSV.SVType = "DEL"
            # artefact FR TD cluster 
            elif clusterC.l_start < clusterC.r_end and clusterC.l_orient == 0 and \
                clusterC.r_orient == 1 and clusterC.lTID == clusterC.rTID and clusterC.isSmall == 1:
                store=0
                newSimpleSV.SVType = "TD"
    ...
    for elem in TDArtefacts:
        ...
        if storeTD:
            ...
            newSimpleSV.SVType = "INS_U"
            newSimpleSV.bp1_end = newSimpleSV.bp2_end # set "both" breakpts same by convention
            newSimpleSV.bp2_start = newSimpleSV.bp1_start
            consolidatedCls_C.append(elem)

    writeVariants(consolidatedCls_C, fVariantsPE, fVariantMapPE, hashedVM, varCount)
    fClustersLS.close()
    fClustersRS.close()
    fVariantsPE.close()
    fClusterMap.close()
